...
export class RequestRouter implements IRequestRouter {
  ...
  async routeRequests(event: APIGatewayProxyEvent): Promise<any> {
    ...
    try {
      if (this.isUserEndpoint(path)) {
        return await this.routeUsersRequests(httpMethod, body, path);
      } else {
        return await this.routePostsRequests(httpMethod, body, path);
      }
    } catch (e) {
  ...
  private async routeUsersRequests(httpMethod, body, path): Promise<any> {
    ...
    && httpMethod.toLowerCase() === GET) {
      ...
      return user;
    } else if (path.toLowerCase().includes(ADD_USER)
  ...
  private async routePostsRequests(httpMethod, body, path): Promise<any> {
    ...
    && httpMethod.toLowerCase() === GET) {
      ...
      return user;
    } else if (path.toLowerCase().includes(ADD_POST)
    && httpMethod.toLowerCase() === POST) {
